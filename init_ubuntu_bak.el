...
(define-key (current-global-map) [remap sp-forward-slurp-sexp] 'enlarge-window-horizontally)
(define-key (current-global-map) [remap sp-forward-barf-sexp] 'shrink-window-horizontally)
(define-key (current-global-map) [remap backward-paragraph] 'shrink-window)
(define-key (current-global-map) [remap forward-paragraph] 'enlarge-window)
...
;; redo
(require 'redo+)
(define-key (current-global-map) [remap negative-argument] 'undo-tree-undo)
...
(define-key (current-global-map) [remap sp-splice-sexp-killing-backward] 'highlight-symbol)
...
;;(setq command-line-default-directory "c:\cygwin64\home\pactera\work")
(cd "~/work/")

;;set mark
(define-key (current-global-map) [remap digit-argument] 'set-mark-command)

;(set-default-font "Ubuntu Mono-12") ;Ubuntu Mono 12")
;(push '(font . "Ubuntu Mono") default-frame-alist)
(set-default-font "Ubuntu Mono 14") ;Ubuntu Mono 12")

;; fix tab problem
(define-key (current-global-map) [remap yas-expand] 'c-indent-line-or-region)
(setq default-tab-width 4)

;; chinese encoding setting begin
...
;; chinese encoding setting end

;; change auto-backup dir
